...
const products = [
  {
    img: '/products/hand-sanitizer.jpg',
  ...
  {
    img: '/products/disinfectant-spray.jpg',
  ...
  {
    img: '/products/alcohol-hand-gel.jpg',
  ...
  {
    img: '/products/disinfectant-wipes.jpg',
  ...
  {
    img: '/products/hand-sanitizer-spray.jpg',
...
function ProductCard({
  ...
  return (
    ...
    >
      ...
      <div
        aria-hidden="true"
        className="absolute inset-0 rounded-3xl bg-linear-to-t from-black/80 group-hover:from-[calc(7/16*100%)] from-15% ring-1 ring-gray-950/10 ring-inset sm:from-25% transition-all"
      />
      <figure className="relative p-10">
        <figcaption className="group-hover:mb-4 group-hover:border-b border-white/20 group-hover:pb-4 transition-all">
          ...
          <p className="text-sm/6 font-medium">
            <span className="bg-linear-to-r from-primary to-ternary bg-clip-text text-transparent">
              {title}
            </span>
          </p>
        </figcaption>
      ...
      >
        {products.map(({ img, name, description, category }, productIndex) => (
          <motion.div
            ...
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            transition={{ delay: productIndex * 0.3 }}
          >
            <ProductCard
              name={name}
              title={category}
...
export function Products({ limit }: { limit?: number }) {
  ...
  return (  
    <div className="overflow-hidden py-32">
      <Container>
        <div ref={setReferenceWindowRef}>
          <Subheading>Available Products</Subheading>
          <Heading as="h3" className="mt-2">
            Explore Our Wide Range of Products.
          </Heading>
        </div>
      </Container>
      <div className="grid lg:grid-cols-4 max-w-2xl lg:max-w-7xl gap-8 my-12 mx-6 lg:mx-auto">
        { (limit ? products.slice(0, limit) : products).map(({ img, name, description, category }, productIndex) => (
          <div key={productIndex} className='flex flex-col gap-3 relative'>
            <div className='absolute z-10 top-4 left-4 flex gap-4'>
              {productIndex == 0 && <div className='px-3 py-1 bg-secondary/50 rounded-3xl border border-primary text-white font-semibold'>New</div>}
              {(productIndex+1)%2 == 0 &&<div className='px-3 py-1 bg-white rounded-3xl text-primary font-semibold'>20% OFF</div>}
            </div>
            <Image src={ img } width={1920} height={1080} alt="image hero brand" className="relative rounded-2xl h-[340px] w-full object-cover shadow-xl border border-gray-200" />
            <div className='flex flex-col gap-1'>
              <h4 className='capitalize font-semibold text-xl text-gray-900'>{ name }</h4>
              <p className="text-sm/6 font-medium">
